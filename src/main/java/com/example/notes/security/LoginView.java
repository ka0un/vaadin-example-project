package com.example.notes.security;

import com.vaadin.flow.component.html.Anchor;
import com.vaadin.flow.component.html.Div;
import com.vaadin.flow.component.html.H1;
import com.vaadin.flow.component.html.Paragraph;
import com.vaadin.flow.component.login.LoginForm;
import com.vaadin.flow.component.orderedlayout.FlexComponent;
import com.vaadin.flow.component.orderedlayout.VerticalLayout;
import com.vaadin.flow.router.BeforeEnterEvent;
import com.vaadin.flow.router.BeforeEnterObserver;
import com.vaadin.flow.router.PageTitle;
import com.vaadin.flow.router.Route;
import com.vaadin.flow.server.auth.AnonymousAllowed;

@Route("login")
@PageTitle("Login | Image Gallery")
@AnonymousAllowed
public class LoginView extends VerticalLayout implements BeforeEnterObserver {

    private final LoginForm login = new LoginForm();

    public LoginView() {
        addClassName("login-view");
        setSizeFull();
        setAlignItems(FlexComponent.Alignment.CENTER);
        setJustifyContentMode(FlexComponent.JustifyContentMode.CENTER);

        // Style the background
        getStyle()
                .set("background", "linear-gradient(135deg, #0f0c29 0%, #302b63 50%, #24243e 100%)")
                .set("min-height", "100vh");

        // Card container
        Div card = new Div();
        card.addClassName("login-card");
        card.getStyle()
                .set("background", "rgba(255, 255, 255, 0.05)")
                .set("backdrop-filter", "blur(20px)")
                .set("-webkit-backdrop-filter", "blur(20px)")
                .set("border", "1px solid rgba(255, 255, 255, 0.1)")
                .set("border-radius", "24px")
                .set("padding", "48px 40px")
                .set("box-shadow", "0 25px 50px rgba(0, 0, 0, 0.3)")
                .set("max-width", "420px")
                .set("width", "100%")
                .set("text-align", "center");

        // Icon/Logo area
        Div iconWrapper = new Div();
        iconWrapper.getStyle()
                .set("width", "80px")
                .set("height", "80px")
                .set("border-radius", "20px")
                .set("background", "linear-gradient(135deg, #667eea 0%, #764ba2 100%)")
                .set("display", "flex")
                .set("align-items", "center")
                .set("justify-content", "center")
                .set("margin", "0 auto 24px auto")
                .set("font-size", "36px");
        iconWrapper.setText("📸");

        // Title
        H1 title = new H1("Image Gallery");
        title.getStyle()
                .set("color", "white")
                .set("font-size", "28px")
                .set("font-weight", "700")
                .set("margin", "0 0 8px 0")
                .set("font-family", "'Inter', 'Segoe UI', sans-serif");

        Paragraph subtitle = new Paragraph("Sign in to access your gallery");
        subtitle.getStyle()
                .set("color", "rgba(255, 255, 255, 0.6)")
                .set("margin", "0 0 32px 0")
                .set("font-size", "14px");

        // Configure login form
        login.setAction("login");
        login.setForgotPasswordButtonVisible(false);
        login.getElement().getThemeList().add("dark");

        // Register link
        Div registerLink = new Div();
        registerLink.getStyle()
                .set("margin-top", "24px")
                .set("text-align", "center");

        Paragraph registerText = new Paragraph("Don't have an account?");
        registerText.getStyle()
                .set("color", "rgba(255, 255, 255, 0.5)")
                .set("margin", "0 0 8px 0")
                .set("font-size", "14px");

        Anchor registerAnchor = new Anchor("register", "Create Account");
        registerAnchor.getStyle()
                .set("color", "#667eea")
                .set("text-decoration", "none")
                .set("font-weight", "600")
                .set("font-size", "14px")
                .set("transition", "color 0.2s ease");

        registerLink.add(registerText, registerAnchor);

        card.add(iconWrapper, title, subtitle, login, registerLink);
        add(card);
    }

    @Override
    public void beforeEnter(BeforeEnterEvent event) {
        if (event.getLocation()
                .getQueryParameters()
                .getParameters()
                .containsKey("error")) {
            login.setError(true);
        }
    }
}
